class CosmosysController < ApplicationController
  ...
  def report
    ...
    if request.get? then
      ...
      if (Setting.plugin_cosmosys['repo_local_path'].blank?) then
        ...
        repodir = "#{Setting.plugin_cosmosys['repo_local_path']}"
        ...
        if (File.directory?(repodir)) then
          if (Setting.plugin_cosmosys['relative_reporting_path'].blank?) then
            # If it is not set, we can not continue
            @output += "Error: the relative path to upload file is not set\n"
          else
            reportingpath = repodir + "/" + Setting.plugin_cosmosys['relative_reporting_path']
            if (File.directory?(reportingpath)) then
              imgpath = repodir + "/" + Setting.plugin_cosmosys['relative_img_path']
              if (File.directory?(imgpath)) then
                ...
                begin
                  ...
                  tmpfile.close
                  comando = "python3 plugins/cosmosys/assets/pythons/IssReports.py #{@project.id} #{reportingpath} #{imgpath} #{root_url} #{tmpfile.path}"
                  require 'open3'
                  print(comando)
    ...
    def git_commit_repo(pr,a_message)
      ...
      if (Setting.plugin_cosmosys['repo_local_path'].blank?) then
        ...
        destdir = "#{Setting.plugin_cosmosys['repo_local_path']}"
        ...
        if (Setting.plugin_cosmosys['repo_server_sync']) then
          # The setting says we must sync with a remote server
          if (Setting.plugin_cosmosys['repo_server_path'].blank?) then
            ...
            remote_url = "#{Setting.plugin_cosmosys['repo_server_path']}"
    ...
    def git_pull_rm_repo(pr)
      ...
      if (Setting.plugin_cosmosys['repo_redmine_sync']) then
        # The setting says we must sync with a remote server
        if not(Setting.plugin_cosmosys['repo_redmine_path'].blank?) then
          redminerepodir = "#{Setting.plugin_cosmosys['repo_redmine_path']}"
